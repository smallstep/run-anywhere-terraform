...
locals {
  ...
  master_creds = {
    ...
    username      = local.rds_configs.master_username
    password      = random_password.initial_master_password.result
    port          = local.rds_configs.port
  }
}
...
resource "aws_db_parameter_group" "smallstep" {
  ...
  family = "aurora-postgresql${split(".", local.rds_configs.engine_version)[0]}"
  ...
  # Setting retention period to be cost effective
  parameter {
    name  = "rds.log_retention_period"
    value = local.backup_retention_period * 1440
  }
}

resource "aws_lambda_function" "make_dbs" {
  filename                       = data.archive_file.make_dbs.output_path
  function_name                   = "${local.default_name}-make-dbs"
  ...
  tags = {
    Name        = "${local.default_name}-make-dbs"
...
resource "aws_iam_role_policy" "make_dbs_2" {
  name = "${local.default_name}-kms-decrypt"
...
resource "aws_iam_role_policy" "make_dbs_3" {
  name = "${local.default_name}-db-secret-access"
  role = aws_iam_role.make_dbs.id
...
resource "aws_rds_cluster" "smallstep" {
  ...
  cluster_identifier        = local.default_name
  copy_tags_to_snapshot     = true
  deletion_protection       = true
  engine                    = "aurora-postgresql"
  engine_mode               = "provisioned"
  engine_version            = local.rds_configs.engine_version
  final_snapshot_identifier = "${local.default_name}-final"
  ...
  master_username = local.rds_configs.master_username
...
resource "aws_rds_cluster_instance" "smallstep" {
  count = local.rds_configs.instance_count

  identifier          = "${local.default_name}-${format("%02d", count.index + 1)}"
  cluster_identifier  = aws_rds_cluster.smallstep.cluster_identifier
  engine              = "aurora-postgresql"
  instance_class      = local.rds_configs.instance_class
  ...
  tags = {
    Name        = "${local.default_name}-${format("%02d", count.index + 1)}"
    Description = local.default_description
  }
...
resource "aws_security_group" "rds" {
  name        = "${local.default_name}-postgres-dbs"
...
resource "aws_security_group_rule" "eks_to_rds" {
  type                     = "ingress"
  from_port                = local.rds_configs.port
  to_port                  = local.rds_configs.port
  protocol                 = "tcp"
